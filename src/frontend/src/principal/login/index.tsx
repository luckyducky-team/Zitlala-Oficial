...
class AzleApp extends LitElement {
    identity: Identity | null; // Declare the 'identity' property
    whoami: string;
    ...
    async whoamiUnauthenticated() {
        const response = await fetch(`${import.meta.env.VITE_CANISTER_ORIGIN}/whoami`);
    ...
    render() {
        return html`
            ...
            <button
                ...
                @click=${this.whoamiUnauthenticated.bind(this)}
            >
                Whoami Unauthenticated
            </button>
            <button
                id="whoamiAuthenticated"
                @click=${this.whoamiAuthenticated.bind(this)}
